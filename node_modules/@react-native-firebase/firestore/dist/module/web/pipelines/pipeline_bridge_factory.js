...
function normalizeWebDistanceMeasure(value, fieldName) {
  ...
  switch (normalized) {
    ...
    case 'DOTPRODUCT':
      return 'dot_product';
    default:
      throw new Error(`pipelineExecute() expected ${fieldName} to be one of euclidean, cosine, or dot_product.`);
  }
}
function normalizeWebFindNearestOptions(options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'distanceMeasure')) {
    return options;
  }
  return {
    ...options,
    distanceMeasure: normalizeWebDistanceMeasure(options.distanceMeasure, 'stage.options.distanceMeasure')
  };
}
...
function applyPipelineStage(current, stage, nestedUnionPipeline) {
  ...
  switch (stage.stage) {
    ...
    case 'findNearest':
      return method.call(current, normalizeWebFindNearestOptions(stageArgs));
    default:
      return method.call(current, stageArgs);
  }
}
